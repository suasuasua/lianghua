...
class BacktestEngine:
    ...
    def run(self, panel: pd.DataFrame, lookback: int = None) -> pd.DataFrame:
        ...
        for idx in rebalance_dates:
            ...
            # Only sell sectors explicitly on the sell list
            ...
            # Buy: fill empty slots up to max_positions, prioritizing buy_list
            ...
            if slots_left > 0 and cash > 0:
                ...
                if len(new_buys) < slots_left:
                    # Fill remaining with highest-ranked hold sectors not already owned
    ...
    def _compute_nav_series(self, panel: pd.DataFrame) -> pd.Series:
        ...
        for i in range(len(panel)):
            ...
            pos_value = sum(
                shares * float(panel.iloc[i][sector])
